## Description
Figure 7  – A) Faceted dot plots displaying amino acid identity percentages for 11 vir genes homologues across pairwise comparisons. Genes are ordered by identity percentage within each subplot, with red X marks indicating genes absent from one plasmid (NA) versus genes present but sharing no identity (0%). The 50% identity threshold (dashed line) is provided for reference. Top row: local plasmid comparisons (CP4, CP5, CP10). Middle row: comparisons to C. jejuni reference strain 81-176 pVir. Bottom row: comparisons to Agrobacterium tumefaciens T4SS. Colours denote comparison category: green (intra-plasmid), orange (C. jejuni reference strain plasmid), purple (Agrobacterium). B) Schematic representation of the T4SS structure prediction in Agrobacterium tumefaciens strain 1D1609 encoded by the virB operon. The structure model was retrieved from BioRender.

### Input

The amino acid sequences of the *virB* operon homologous genes were obtained from the NCBI protein database for the reference sequences *A. tumefaciens* and the virulence plasmid sequence from *C. jejuni* 81-176. The amino acid sequences of the *virB* homologous genes present in one representative plasmid from CP4, CP5, and CP10 types were extracted from their annotations. BLASTp was used to compare the amino acid sequences and the percentage of identity was the input for the script herein used.

[T4SS BLASTp comparison](./Fig06_T4SS_comparison.csv)

### Script for Figure 07

The following script generates a dotted plot displaying the BLASTp results:
...
[Figure 06](./t4ss_faceted_dotplot.pdf)
