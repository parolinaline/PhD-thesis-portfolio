...
## Campylobacter coli - efflux pump allelic diversity chapter
## ============================================================
## Workflow:
##   1. Load trees
...
snp_tree  <- read.tree("coli_cgMLST.nwk")
cmeA_tree <- read.tree("cmeA_coli.treefile")
cmeB_tree <- read.tree("cmeB_coli.treefile")
cmeC_tree <- read.tree("cmeC_coli.treefile")
cmeR_tree <- read.tree("cmeR_coli.treefile")
...
cat("\n=== TIP LABEL SUMMARY ===\n")
cat("SNP tree tips:  ", length(snp_tree$tip.label), "\n")
...
cat("\n=== EXCLUDED ISOLATES (present in SNP tree but missing from gene tree) ===\n")
for (gene in names(gene_trees)) {
...
cat("\n=== ISOLATES IN GENE TREES BUT NOT IN SNP TREE (unexpected) ===\n")
for (gene in names(gene_trees)) {
...
for (gene in names(gene_dend)) {
  ...
  outfile <- paste0("tanglegram_cgMLST_vs_", gene, "_ccoli.pdf")
  ...
  tanglegram(
    ...
    ## ── Tip alignment lines (the ones we wanted to remove) ──
    dLeaf          = -0.01,    # negative value pulls tips away from centre,
                               # effectively hiding the alignment dashes
    ## if dashes still visible, also set:
    # edge.root.lwd  = 0,      # remove root edge

    ## ── Headers ─────────────────────────────────────────────
    main           = paste("cgMLST tree  vs ", gene, " tree — C. coli"),
    main_left      = "cgMLST",
    main_right     = gene,
    cex.main       = 1.1,
...
write.csv(rf_results, "ccoli_RF_distances_cgMLST.csv", row.names = FALSE)
cat("\nRF distances saved to: ccoli_RF_distances_cgMLST.csv\n")
...
rf_plot <- ggplot(rf_results, aes(x = gene, y = RF_norm, fill = gene)) +
  ...
  labs(
    ...
    subtitle = "C. coli — cme efflux pump genes",
    x        = "Gene tree",
...
rf_plot
ggsave("ccoli_RF_barplot_cgMLST.pdf", rf_plot, width = 7, height = 5)
cat("RF barplot saved to: ccoli_RF_barplot_cgMLST.pdf\n")
