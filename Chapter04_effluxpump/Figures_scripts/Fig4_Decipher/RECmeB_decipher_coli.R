...
input_file <- "CmeB_alleles_coli.faa"
...
full_html <- "REcmeB_fullalignment_coli.html"
...
#  names(region_seqs) <- paste0(names(region_seqs), "  [CmeB positions 136-303]")
  ...
  region_html <- "REcmeB_coli_136_303.html"
  
  BrowseSeqs(region_seqs,
             htmlFile = region_html,
             openURL  = FALSE,
             title    = "CmeB alignment C. coli — positions 136 to 303 (RE-CmeB mutation sites)")
...
ggsave("RE_cmeB_mutation_coli.pdf", p, width = 10, height = 5)
...
writeXStringSet(aligned_sequences, "REcmeB_sequences_coli.fasta")
cat("Aligned FASTA saved: REcmeB_sequences_coli.fasta\n")

write.csv(mutation_table,  "REcmeB_mutation_coli.csv",   row.names = FALSE)
cat("Mutation table saved: REcmeB_mutation_coli.csv\n")

write.csv(annotations, "REcmeB_annotation_coli.csv", row.names = FALSE)
cat("Full annotations saved: REcmeB_annotation_coli.csv\n")
